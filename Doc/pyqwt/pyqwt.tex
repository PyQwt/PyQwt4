...
\newcommand{\Sip}{\ulink{sip-4.4.3.tar.gz}
  {http://pyqwt.sourceforge.net/support/sip-4.4.3.tar.gz}}
...
\newcommand{\NumPyTarGz}{\ulink{NumPy-0.9.8.tar.gz}
  {http://prdownloads.sourceforge.net/numpy/numpy-0.9.8.tar.gz?download}}
\newcommand{\numarray}{\ulink{numarray}
  {http://www.stsci.edu/resources/software_hardware/numarray}}
\newcommand{\numarrayTarGz}{\ulink{numarray-1.5.1.tar.gz}
  {http://prdownloads.sourceforge.net/numpy/numarray-1.5.1.tar.gz?download}}
\newcommand{\Numeric}{\ulink{Numeric}
  {http://www.numpy.org}}
\newcommand{\NumericTarGz}{\ulink{Numeric-23.8.tar.gz}
  {http://prdownloads.sourceforge.net/numpy/Numeric-23.8.tar.gz?download}}
\newcommand{\PyQwtTarGz}{\ulink{PyQwt-\version.tar.gz}
  {http://prdownloads.sourceforge.net/pyqwt/PyQwt-\version.tar.gz?download}}
\newcommand{\ReadMePatches}{\ulink{README}
  {http://pyqwt.sourceforge.net/patches/README}}

% Installers for MS-Windows
\newcommand{\PythonExe}{\ulink{Python-2.4.3.exe}
  {http://www.python.org/ftp/python/2.4.3/Python-2.4.3.exe}}
\newcommand{\NumericExe}{\ulink{Numeric-23.8.win32-py2.4.exe}
  {http://prdownloads.sourceforge.net/numpy/Numeric-23.8.win32-py2.4.exe?download}}
\newcommand{\numarrayExe}{\ulink{numarray-1.5.1.win32-py2.4.exe}
  {http://prdownloads.sourceforge.net/numpy/numarray-1.5.1.win32-py2.4.exe?download}}
...
\newcommand{\PrerequisitesEnd}{
  ...
  library with a patch to force an update of the axis scales after zooming.
  You have the option to compile and link those sources statically into PyQwt
  ...
  \NumPy{} is actively developed and recommended (although in a state of flux).
...
\begin{itemize}
\item
  subsection \ref{iqt-intro} and \ref{intro-qplt-plot} the modules
  \module{Qwt4.iqt} and \module{Qwt4.qplt} which facilite the use of PyQwt from
  the command line interpreter.
\item
  subsection \ref{qwt-qimage} describes the functions to convert
  \class{QImage} instances to NumPy/numarray/Numeric arrays and vice versa.
\end{itemize}
...
\begin{notice}[warning]
  The module \module{Qwt4.iqt} will not yet work with a MSVC-compiled Python,
...
Module \module{Qwt4.iqt} enables you to:
\begin{itemize}
\item
...
Module \module{Qwt4.iqt} works by hooking \cfunction{qApp->processEvents()} on
...
My \file{.pythonrc.py} file loads the modules \module{readline} and
\module{rlcompleter}.  Then, it configures \module{readline} for tab-completion
and history saving and cleans up the global namespace.
Finally, it loads the modules \module{numpy}, \module{scipy}, \module{qt},
\module{Qwt4.iqt}, \module{Qwt4.Qwt} and \module{Qwt4.qplt}:
...
\verbatiminput{Qwt4.qplt.example}
...
\begin{funcdesc}{to_np_array}{image}
  ...
  building PyQwt. Use \function{toNumeric(aQImage)}
\end{funcdesc}
...
\begin{notice}[note]
  You can install PyQwt-4.2.2 compiled for Qt3 side by side with PyQwt5
  compiled for Qt3 and Qt4 (after the release of PyQwt5).
\end{notice}
...
\begin{enumerate}
  ...
  \item
    Either \PyQtGpl{}, or \PyQtMac{}.\\
    Supported versions: PyQt-3.16.x.
  \item
...
make install
\end{verbatim}
  to link the Qwt library dynamically with PyQwt.
...
\begin{classdesc*}{QWidgetIntDictIt}
  is a wrapper for \ctype{QWidgetIntDictIt} and cannot be instantianated from
...
\begin{classdesc*}{QwtArrayLong}
  is a wrapper for \ctype{QwtArray<long>}, see \ref{qwtarray}.
\end{classdesc*}

\begin{classdesc*}{QwtArrayQwtDoublePoint}
  is a wrapper for \ctype{QwtArray<QwtDoublePoint>}, see \ref{qwtarray}.
\end{classdesc*}
...
autoScale.adjust(x, reset=0)
    \end{verbatim}
    where \var{x} can be a Numerical Python array or a list or tuple
    convertible to a Numerical Python array and \var{reset} is a boolean.
  \end{cfuncdesc}
...
\begin{classdesc*}{QwtPlotCurveIterator}
  is a wrapper for \ctype{QwtPlotCurveIterator} and cannot be instantianated
...
\begin{classdesc*}{QwtPlotMarkerIterator}
  is a wrapper for \ctype{QPlotMarkerIterator} and cannot be instantianated
  from Python, see \ref{dictit}.
\end{classdesc*}
...
success = spline.recalc(x, y, periodic)
    \end{verbatim}
    where \var{x} and \var{y} can be any combination of lists, tuples and
    Numerical Python arrays and \var{periodic} is a boolean.
...
\item
  \class{QwtArrayLong} for \ctype{QwtArray<long>}
\item
...
\verbatiminput{Qwt4.QwtArrayDouble.example}
...
\item
  \class{QwtCurveDictIterator}
\item
  \class{QwtMarkerDictIterator}
\end{itemize}
...
\newcommand{\DictIt}[1]{
  \begin{classdesc*}{#1}
    \begin{methoddesc}[#1]{count}{}
      Returns the number of items in the dictionary over which the iterator is
      operating. 
    \end{methoddesc}

    \begin{methoddesc}[#1]{current}{}
      Returns the current item of the iterator.
    \end{methoddesc}

    \begin{methoddesc}[#1]{isEmpty}{}
      Returns \code{True} if the dictionary is empty, i.e. count() == 0;
      otherwise returns \code{False}.
    \end{methoddesc}

    \begin{methoddesc}[#1]{toFirst}{}
      Resets the iterator, making the first item the first current item. First
      in this context means first in the arbitrary traversal order. Returns
      this item.
      
      If the dictionary is empty it sets the current item to \code{None} and
      returns \code{None}.
    \end{methoddesc}

    \begin{methoddesc}[#1]{next}{}
      ...
      was \code{None}, \code{None} is returned. 
    \end{methoddesc}
  \end{classdesc*}
}

\DictIt{QWidgetIntDictIt}

\DictIt{QwtPlotCurveIterator}

\DictIt{QwtPlotMarkerIterator}
